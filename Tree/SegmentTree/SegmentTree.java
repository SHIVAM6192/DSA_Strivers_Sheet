...
public class SegmentTree {
    ...
    public Node constructTree(int[] arr, int start, int end){
        if(start == end){
            // if start and end are same it means we are at leaf node
        ...
        root.left = this.constructTree(arr, start, mid);
        root.right = this.constructTree(arr, mid + 1, end);

        root.data = root.left.data + root.right.data;
        return root;
    }

    // Query on given start and end index
